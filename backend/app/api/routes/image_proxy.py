...
class ProxyRequest(BaseModel):
    url: str  # Changed from HttpUrl to str to accept relative paths
...
async def proxy_fetch_image(payload: ProxyRequest, request: Request):
    ...
    if src_url.startswith("/"):
        # Remove query parameters (e.g., ?t=timestamp for cache busting)
        parsed_path = urlparse(src_url)
        clean_path = parsed_path.path  # This removes query string

        # Convert relative path to absolute path relative to current working directory
        # Remove leading slash and use current working directory as base
        relative_from_root = clean_path.lstrip("/")
        local_src = os.path.join(os.getcwd(), relative_from_root)

        # Normalize path separators for Windows
        local_src = os.path.normpath(local_src)

        if not os.path.exists(local_src):
            raise HTTPException(
                status_code=404, detail=f"파일을 찾을 수 없습니다: {local_src}"
            )
    ...
    if same_host and same_port and parsed.path.startswith("/api/"):
        try:
            ...
            if resp.status_code != 200:
                raise HTTPException(
                    status_code=502,
                    detail=f"원격 이미지를 가져오지 못했습니다: {resp.status_code}",
                )
    ...
    try:
        ...
        raise HTTPException(
            status_code=504, detail="원격 이미지 응답 지연으로 타임아웃"
        )
    except Exception as e:
