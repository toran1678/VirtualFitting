...
class VirtualFittingServiceRedis:
    def __init__(self):
        ...
        self.ootd_model_path = (
            self.project_root / "app" / "api" / "ml_models" / "OOTDiffusion" / "run"
        )
    ...
    def process_virtual_fitting_task(self, task_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            process = (
                db.query(VirtualFittingProcess)
                .filter(VirtualFittingProcess.id == process_id)
                .first()
            )
            ...
            for scale_try, samples_try in attempts:
                try:
                    logger.info(
                        f"가상 피팅 시도: scale={scale_try}, samples={samples_try}"
                    )
                    ...
                    if (
                        ...
                        or "out of memory" in err_msg.lower()
                    ):
                        logger.warning(
                            f"GPU 메모리 관련 오류 감지, 재시도 진행: {err_msg}"
                        )
                        continue
                    else:
                        logger.error(
                            f"가상 피팅 실행 실패(재시도 불가 오류): {err_msg}"
                        )
                        break
                ...
                if i < 6:  # 최대 6개까지
                    ...
                    if relative_path.startswith("uploads/"):
                        ...
                        if "uploads" in path_obj.parts:
                            ...
                            logger.warning(
                                f"절대 경로를 상대 경로로 수정: {relative_path} -> {corrected_path}"
                            )
                        else:
    ...
    def _run_ootd_diffusion(
        ...
        if not self.ootd_model_path.exists():
            raise FileNotFoundError(
                f"OOTDiffusion 모델 경로를 찾을 수 없습니다: {self.ootd_model_path}"
            )

        run_ootd_path = self.ootd_model_path / "run_ootd.py"
        if not run_ootd_path.exists():
            raise FileNotFoundError(
                f"run_ootd.py 파일을 찾을 수 없습니다: {run_ootd_path}"
            )
        ...
        if result.returncode != 0:
            raise Exception(
                f"OOTDiffusion 실행 실패 (코드: {result.returncode}): {result.stderr}"
            )
    ...
    def _move_result_images(
        self, unique_id: str, model_type: str, samples: int
    ) -> List[str]:
    ...
    def get_fitting_status(
        self, db: Session, process_id: int, user_id: int
    ...
    def select_fitting_result(
        ...
        selected_image_path = getattr(
            process, f"result_image_{selected_image_index}", None
        )
        ...
        try:
            ...
            fitting_result = VirtualFittings(
                ...
                source_model_image_url=self._normalize_to_relative(
                    process.model_image_path
                ),
                source_cloth_image_url=self._normalize_to_relative(
                    process.cloth_image_path
                ),
            ...
            logger.info(
                f"가상 피팅 결과 선택 완료 및 프로세스 삭제: process_id={process_id}, fitting_id={fitting_result.fitting_id}"
            )
    ...
    def _copy_selected_image_to_final_location(
        self, temp_image_path: str, user_id: int
    ) -> Optional[str]:
        """선택된 이미지를 최종 저장소로 복사"""
        try:
            # 최종 저장 디렉토리 생성
    ...
    def _cleanup_unselected_images(
        self, process: VirtualFittingProcess, selected_index: int
    ):
        """선택되지 않은 이미지들 정리"""
        for i in range(1, 7):  # result_image_1 ~ result_image_6
            if i != selected_index:
